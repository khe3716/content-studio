...
function buildSaeopPrompt(ctx, roundNum, hogigsimOutput, previousSaeopBusinesses, retryHint) {
  const base = [
    ...
    '## 사용자 자산·비자산 (반드시 의식)',
    '진짜 자산:',
    '- 1년치 스마트스토어 운영 데이터 (정산·반품·CS·매출 변동)',
    '- 과일 도메인 지식 (산지·시즌·품질 평가)',
    '- 1년차 1인 사장의 시야 (지금 시작하는 동료에게 가장 적실한 정보)',
    '- 본인의 실제 매출·실패 경험 (투명 공개 시 신뢰도 자산)',
    '비자산 (오해 금지):',
    '- 인스타·트위터·유튜브 팔로워 = **0**',
    '- 뉴스레터 구독자 = **0**',
    '- 블로그 트래픽 = 미미 (애드센스 심사 중)',
...
function buildHyeonsilPrompt(ctx, roundNum, hogigsimOutput, saeopOutput, retryHint) {
  const base = [
    ...
    '7. **도달 부트스트랩 (신규)**: 사용자 도달 0 → 첫 100명까지 3단계(1~2주차 / 3~6주차 / 7~12주차) 채널·콘텐츠·기간 명시 여부. 단계 누락 시 무조건 반려.',
    '8. **단위 경제학 (신규)**: 단가 vs CAC, BEP, 수수료 반영, 가정 환각 검증. CAC > 단가 50% 이상이면 무조건 반려.',
    '',
    '## 요청',
...
function buildGyeoljaePrompt(date, allRounds, ctx) {
  ...
  return [
    ...
    '',
    '## 사용자 자산·비자산 (반드시 의식)',
    '진짜 자산: 1년치 운영 데이터 / 과일 도메인 지식 / 1년차 사장 시야 / 본인 매출·실패 경험',
