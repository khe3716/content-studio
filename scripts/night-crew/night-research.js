...
function buildHogigsimPrompt(ctx, roundNum, totalRounds, previousTopics, retryHint) {
  const base = [
    ...
    '',
    '## 🚫 절대 제외 도메인 (사용자 본업 — 야간팀이 침범하면 안 됨)',
    '- 스마트스토어/쿠팡/네이버 셀러 대상 사업 (운영 노하우·CS·정산·반품·SEO 도구·템플릿·가이드)',
    '- 과일 도매·소매 직접 판매',
    '- 과일 블로그·스마트스토어 SEO·마케팅 도구',
    '위 영역에서는 케이스를 가져오지 마세요. **다른 도메인의 1인 사업 케이스만** 발굴.',
    '',
    '## 요청',
...
function buildSaeopPrompt(ctx, roundNum, hogigsimOutput, previousSaeopBusinesses, retryHint) {
  const base = [
    ...
    '## 사용자 자산·비자산 (반드시 의식)',
    '진짜 자산 (활용 가능):',
    '- **과일 도메인 지식** (산지·시즌·품질·소비자 행동) — 일반 소비자/요리/식문화 영역에 활용',
    '- **1년차 1인 사장의 시야·사고 패턴** — 시간 관리·의사결정·재정 관리를 **다른 도메인** 1인 사업자에게 일반화',
    '- 본인의 실제 매출·실패 경험 — 단, 스마트스토어 외 영역에서 진솔성 자산',
    '- 한국어·비개발자·핸드폰 선호',
    '',
    '🚫 **절대 금지 후보 (사업명·타겟·상품 중 하나라도 해당하면 자동 반려)**:',
    '- 스마트스토어/쿠팡/네이버 셀러 대상 사업 (운영 노하우 키트·CS 템플릿·정산 시트·반품 응대·상세페이지 프롬프트·SEO 도구 등)',
    '- 스마트스토어 운영 데이터 직접 상품화 일체',
    '- 과일 도매·소매 직접 판매·과일 블로그/스토어 SEO·마케팅 도구',
    '야간팀의 존재 이유 = 본업과 분리된 새 영역 탐색. 본업 도메인 우려먹기 금지.',
    '',
    '비자산 (오해 금지):',
    '- 인스타·트위터·유튜브 팔로워 = **0**',
    '- 뉴스레터 구독자 = **0**',
...
function buildHyeonsilPrompt(ctx, roundNum, hogigsimOutput, saeopOutput, retryHint) {
  const base = [
    ...
    '',
    '## 🚫 추가 자동 반려 (사용자 본업 도메인 침범)',
    '- 스마트스토어/쿠팡/네이버 셀러 대상 사업',
    '- 스마트스토어 운영 노하우 직접 상품화 (CS·정산·반품·상세페이지 프롬프트 등)',
    '- 과일 도매·소매 직접 판매·과일 블로그/스토어 SEO·마케팅 도구',
    '사업명·타겟 고객·상품 중 하나라도 해당하면 → 무조건 ❌ 반려',
    '',
    '## 요청',
...
const HYEONSIL_RETRY_HINTS = [
  ...
  '지난 응답도 빈약했습니다. 8대 기준(자본·시간·시장 포화·기술 난도·수익화 속도·참고 케이스·도달 부트스트랩·단위 경제학) 중 5개 이상 체크하세요. 위험은 구체 근거로.',
];
...
function buildGyeoljaePrompt(date, allRounds, ctx) {
  ...
  return [
    ...
    '',
    '## 사용자 자산·비자산 (반드시 의식)',
    '진짜 자산: 과일 도메인 지식(소비자 대상) / 1년차 1인 사장 시야·사고 패턴(다른 도메인에 일반화) / 본인 경험 / 한국어·비개발자',
    '비자산: 팔로워 0 / 구독자 0 / 블로그 트래픽 미미 / 인플루언서 네트워크 없음 / 자본 0~10만원',
    '"기존 팬덤 활용" 같은 빈말 금지. 도달 부트스트랩(0→100명 3단계) 명시 안 된 후보는 ★ 이하 강등.',
    '',
    '## 🚫 자동 보류 (본업 도메인 침범 — 후보 자격 없음)',
    '- 스마트스토어/쿠팡/네이버 셀러 대상 사업',
    '- 스마트스토어 운영 노하우 직접 상품화 (CS·정산·반품·상세페이지 프롬프트 등)',
    '- 과일 도매·소매 직접 판매·과일 블로그/스토어 SEO·마케팅 도구',
    '서사업이 위 영역 후보를 올렸거나 구현실이 통과시켰어도 박결재가 자동 보류로 분류.',
    '',
    '## 요청',
    '1. 구현실 ❌ 반려 건 자동 제외',
    '2. 중복 사업 통합',
    '3. 자본 10만원 초과 건 보류 (Apple $99 포함 체크)',
    '4. 별점 산정 후 **상위 2~3개로 압축** (이전 5개 → 깊이 5배)',
    '5. 오늘 한 건 지정 (★★★ 중 자본 0원·도달 부트스트랩 가장 구체적인 것)',
    '6. **4페이지 풀 사업계획서 작성** (총 6,000~8,000자)',
    '',
    '## 후보 디테일 (필수 A~J 10개 섹션)',
    '각 후보 1.5~2페이지 분량 (1,500~2,500자). raw 응답에서 인용. 부족하면 "(추정)"·"(미확정)" 표기, 빈 섹션 금지.',
    '- **A. 사업 개요** 3~5줄: 무엇·누구·왜·차별화 한 단락',
    '- **B. 참고 성공 사례** 3~5줄: 사업명/운영자/플랫폼/매출 + **운영자 출발점(0인지 팔로워 N만+인지)** + 핵심 메커니즘',
    '- **C. 시장 규모** 4~6줄: TAM(전체) / SAM(도달 가능) / SOM(도달 0 사용자가 1년 안에 잡을 수 있는 현실 N명) / 시장 흐름',
    '- **D. 경쟁자 분석** 5~8줄: 경쟁자 2~3개(이름·플랫폼·가격·강점·약점) + **사용자 wedge(차별화 빈틈)**',
    '- **E. 가격 전략** 3~5줄: 권장 가격 + 근거 + 진화 로드맵',
    '- **F. 도달 부트스트랩 (가장 자세히)** 8~12줄: 1단계(1~2주차 무자본 시드) / 2단계(3~6주차 첫 매출) / 3단계(7~12주차 채널 자산화) — 채널 이름·콘텐츠·기간·전환율 가정 명시',
    '- **G. 단위 경제학** 4~6줄: 단가 / 변동 비용 / 마진/건 / CAC / LTV / BEP',
    '- **H. 실행 로드맵** 8~12줄: 1주~3주 / 1개월 / 3개월 / 1년 마일스톤',
    '- **I. 핵심 위험 + 완화** 3~5줄: 위험 3개 각각 mitigation',
    '- **J. 첫 수익 시점·금액** 1~2줄: 보수적 / 목표 시나리오',
    '',
    '## 분량·형식',
    '- 총 6,000~8,000자 (A4 4페이지 PDF). 텔레그램은 3,800자에서 자동 잘림 + PDF 풀 표시.',
    '- 후보 2~3개로 압축. 후보 4개+면 깊이가 떨어짐 → 절제.',
    '- 보류·반려 섹션은 한 줄씩 짧게.',
    '',
    '## 참고 사례 누락 시',
    '서사업이 참고 케이스 명시 안 했거나 구현실이 6/7/8번 중 하나라도 반려한 후보는 ★ 이하 강등 또는 보류로 분류.',
    '',
...
(async () => {
  ...
  for (let r = 1; r <= opts.rounds; r += 1) {
    ...
    const hogigsimRes = await callWithRetry('lee-hogigsim', (hint) =>
      buildHogigsimPrompt(ctx, r, opts.rounds, memory.snapshot(), hint), {
        ...
        geminiOpts: { temperature: 0.6, maxTokens: 12288 },
      });
    ...
    const saeopRes = await callWithRetry('seo-saeop', (hint) =>
      buildSaeopPrompt(ctx, r, round.hogigsim, allSaeopBusinesses, hint), {
        ...
        geminiOpts: { temperature: 0.5, maxTokens: 16384 },
      });
    ...
    const hyeonsilRes = await callWithRetry('gu-hyeonsil', (hint) =>
      buildHyeonsilPrompt(ctx, r, round.hogigsim, round.saeop, hint), {
        ...
        geminiOpts: { temperature: 0.4, maxTokens: 12288 },
      });
  ...
  const gyeoljaeRes = await callWithRetry('park-gyeoljae', (hint) => buildGyeoljaePrompt(date, allRounds, ctx), {
    ...
    geminiOpts: { temperature: 0.3, maxTokens: 24576 },
  });
